...
def test_hero_instance():
    Athena = superheroes.Hero("Athena", 100)
    assert Athena
...
def test_hero_ability_attack_standard_deviation():
    willow_waffle = superheroes.Hero("Willow Waffle", 150)
...
def test_hero_weapon_equip():
    sans = superheroes.Hero("Comic Sans", 180)
...
def test_hero_weapon_attack_mean_value():
    kkrunch = superheroes.Hero("Kaptain Krunch", 250)
    strength = random.randint(10, 30000)
...
def test_team_hero():
    team = superheroes.Team("One")
    jodie = superheroes.Hero("Jodie Foster", 300)
    team.add_hero(jodie)
